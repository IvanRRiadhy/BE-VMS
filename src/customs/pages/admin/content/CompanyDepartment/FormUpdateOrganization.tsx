...
import { updateOrganization, getVisitorEmployee } from 'src/customs/api/admin';
  ...
  const onSubmit = async (form: any) => {
    ...
    try {
      ...
      if (isBatchEdit && selectedRows.length > 0) {
        const results = await Promise.allSettled(
          selectedRows.map(async (item) => {
            ...
            await updateOrganization(item.id, payload, token);
          }),
        );
        ...
        showSwal(
          ...
          failed.length > 0
            ? 'Some organizations failed to update.'
      ...
      if (data) {
        ...
        await updateOrganization(data.id, payload, token);

        showSwal('success', 'Organization updated successfully.');
        setTimeout(() => onSuccess?.(), 800);
      }
    } catch {
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {/* CODE */}
        <Box display="flex" justifyContent="space-between">
          <CustomFormLabel required>Organization Code</CustomFormLabel>
          {isBatchEdit && (
            <Switch
              size="small"
        ...
        {/* HOST */}
        <Box display="flex" justifyContent="space-between">
          <CustomFormLabel required>Head Of Organization</CustomFormLabel>
          {isBatchEdit && (
            <Switch
              size="small"
