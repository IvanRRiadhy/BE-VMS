...
const View = () => {
  ...
  const fetchRelatedVisitorsByInvitationId = async (invitationId: string) => {
    ...
    const mappedVisitors = relatedData.map((v: any) => ({
      ...
      vehicle_plate_number: v.vehicle_plate_number ?? null,
              ...
              question_page: questionPageTemplate.map((templateSection: any) => ({
                ...
                form: (templateSection.form || []).map((templateField: any, fIdx: number) => {
                  ...
                  if ([10, 11, 12].includes(templateField.field_type)) {
                    ...
                    fieldPayload.answer_text = answer_text ?? null;
                  }
  ...
  const handleCheckboxChange = (item: any, isChecked: boolean) => {
    setSelectedVisitors((prev: string[]) => {
      if (selectMultiple) {
        if (isChecked) {
        ...
        if (isChecked) {
          handleSelectRelatedVisitor(item);
          return [item.id];
        } else {
          return [];
        }
